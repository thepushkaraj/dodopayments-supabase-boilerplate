...
// async function handleSubscriptionEvent(supabase, data, status) {
...
//   let customerRecord = await supabase.from('customers').select('id').eq('dodo_customer_id', customer.customer_id).single();
...
//   const { data: existing } = await supabase.from('subscriptions').select('id').eq('dodo_subscription_id', data.subscription_id).single();
//   if (existing) {
...
// async function handlePaymentEvent(supabase, data, status) {
//   const customer = data.customer;
...
//   }
//   // Find customer
//   const { data: customerRecord } = await supabase.from('customers').select('id').eq('dodo_customer_id', customer.customer_id).single();
//   if (customerRecord) {
